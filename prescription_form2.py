...
def generateFields(sentenceList):
    #print("HELLO")
...
# =============================================================================
    ...
    ageFlag=0
    for sents in sentenceList:
         if (("years old" in sents) or ("years of age" in sents) or ("age is" in sents) or ("age of the patient" in sents)  or ("patient age" in sents) or ("age of patient" in sents) or ("Age is" in sents) or ("Age of the patient" in sents)  or ("Patient age" in sents) or ("Age of patient" in sents)):
             ageFlag=1
             age = age +' '+ sents
    ...
    if(ageFlag==1):       
        wordList=stanfordNER2.posTagger(age)
    ...
    #print(nltk.sent_tokenize(chiefComplaint))
    if(ccFlag==1):
        ...
        for conceptGT10 in UMLSconceptListGT10CC:
             if ((("sosy" in conceptGT10[5]) or ("dsyn" in conceptGT10[5]) or ("fndg" in conceptGT10[5]) or ("inpo" in conceptGT10[5]) or ("hops" in conceptGT10[5])) and (conceptGT10[6][-2]=='0')):
                 chiefComplaint2=chiefComplaint2+conceptGT10[3]+', '
    #print(chiefComplaint2)
    ...
    hpiFlag=0
    for sents in sentenceList:
         if (("history of illness" in sents) or ("history of present illness" in sents) or ("illness history" in sents) or ("hpi" in sents) or ("History of illness" in sents) or ("History of present illness" in sents) or ("Illness history" in sents) or ("Hpi" in sents) or ("HPI" in sents)):
             hpiFlag=1
             hpi= hpi+' '+sents
    #print(nltk.sent_tokenize(chiefComplaint))
    if(hpiFlag==1):
        UMLSconceptListGT10HPI=metamap2.metamapExtract(sent_tokenize(hpi))
    ...
    sz=0
    for sents in sentenceList:
        if (("past medical history" in sents) or ("pmh" in sents) or ("medical history" in sents) or ("Past medical history" in sents) or ("Pmh" in sents) or ("Medical history" in sents) or ("PMH" in sents)):
            pmh=pmh+' '+sents
            if "asthma" in sents:
                asthma=1
            if "copd" in sents:
    ...
    medsFlag=0
    for sents in sentenceList:
         if (("medication" in sents) or ("medicine given" in sents) or ("medicine applied" in sents) or ("Medication" in sents) or ("Medicine given" in sents) or ("Medicine applied" in sents)):
             medsFlag=1
             meds= meds+' '+sents
    ...
    if(medsFlag==1):
        ...
        for conceptGT10 in UMLSconceptListGT10MEDS:
             if ((("orch" in conceptGT10[5]) or ("phsu" in conceptGT10[5]))):
    ...
    for sents in sentenceList:
        if (("allergies" in sents) or ("Allergies" in sents)):
    ...
    perxtxFlag=0
    for sents in sentenceList:
        if (("medication" in sents) or ("given medicine" in sents) or ("given medication" in sents) or ("medicine given" in sents) or ("medicine applied" in sents) or ("Medication applied" in sents) or ("Given medicine" in sents) or ("Given medication" in sents) or ("Medicine given" in sents) or ("Medicine applied" in sents) or ("Medication applied" in sents) or ("Medication" in sents) or ("Medication administrated" in sents) or ("medication administrated" in sents) or ("Administrated medicine" in sents)):
            perxtxFlag=1
            perxtx=perxtx+' '+sents
    ...
    if(primaryInitialVitalSignsFlag==1):
        ...
        if ('glucose' in wordTok):
            primaryInitialVitalSigns.insert(1,str(wordTok[(wordTok.index('glucose')+1)]))
        if ('gcse' in wordTok):
            primaryInitialVitalSigns.insert(2,str(wordTok[(wordTok.index('gcse')+1)]))
        if ('gcsv' in wordTok):
            primaryInitialVitalSigns.insert(3,str(wordTok[(wordTok.index('gcsv')+1)]))
        if ('gcsm' in wordTok):
            primaryInitialVitalSigns.insert(4,str(wordTok[(wordTok.index('gcsm')+1)]))
        if ('equals' in wordTok):
            primaryInitialVitalSigns.insert(5,str(wordTok[(wordTok.index('equals')+1)]))
        if ('respiratory' in wordTok):
            primaryInitialVitalSigns.insert(6,str(wordTok[(wordTok.index('respiratory')+1)]))
        if ('pressure' in wordTok):
            primaryInitialVitalSigns.insert(7,str(wordTok[(wordTok.index('pressure')+1)]))
        if ('ekg' in wordTok):
            primaryInitialVitalSigns.insert(8,str(wordTok[(wordTok.index('ekg')+1)]))
        if ('pulse' in wordTok):
            primaryInitialVitalSigns.insert(9,str(wordTok[(wordTok.index('pulse')+1)]))
        if ('spo2' in wordTok):
            primaryInitialVitalSigns.insert(10,str(wordTok[(wordTok.index('spo2')+1)]))
        if ('etco2' in wordTok):
            primaryInitialVitalSigns.insert(11,str(wordTok[(wordTok.index('etco2')+1)]))
        if ('temp' in wordTok):
            primaryInitialVitalSigns.insert(12,str(wordTok[(wordTok.index('temp')+1)]))
...
# =============================================================================
    ...
    for sents in sentenceList:
         if (("Procedures taken" in sents) or ("procedures taken" in sents)):
             proceduresFlag=1
    ...
    if(proceduresFlag==1):
        ...
        for i in range(numOfProcedures):
            sentence=str(sntnceLst[i])
            #print(sentence)
            wordTok=word_tokenize(sentence)
            
            if (('procedure' in wordTok) and ('name' in wordTok) and ( wordTok.index('name') == wordTok.index('procedure')+1 ) ):
                tempSntnce=''
                for ii in range(100):
                    tempSntnce=tempSntnce+' '+str(wordTok[(wordTok.index('name')+ii+1)])
                    if(str(wordTok[(wordTok.index('name')+ii+2)])==','):
                        break 
                proceduresTaken[i].insert(0,tempSntnce)
            if ('location' in wordTok):
                tempSntnce=''
                for ii in range(100):
                    tempSntnce=tempSntnce+' '+str(wordTok[(wordTok.index('location')+ii+1)])
...
#    pdf.multi_cell(0, 18, 'PMH:  ASTHMA   COPD   CHF   CAD   MI   RENAL FAILURE   CVA   DIABETES   HTN   SZ\n'+pmh, 1, 1, 'J')
    ...
    pdf.set_text_color(0,0,0)
    pdf.multi_cell(0, 8, 'MEDS: '+meds2, 1, 1, 'L')
    pdf.multi_cell(0, 8, 'ALLERGIES: '+allergies, 1, 1, 'J')
    pdf.multi_cell(0, 10, 'PE/RX/TX: '+perxtx2, 1, 1, 'J')
    ...
    for i in range(numOfSecondaryVitalSigns):
        ...
        pdf.cell(20, 7, secondaryInitialVitalSigns[i][6], 1, 0, 'C')
